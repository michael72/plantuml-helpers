...
describe("Rotate", () => {
  it("should rotate arrows", () => {
    ...
    expect(
      r.rotateLine("A --> B : hello", r.RotateDirection.Swap)
    ).toBe("B <-- A : hello");
    expect(
      r.rotateLine("  A --> B : hello", r.RotateDirection.Right)
    ).toBe("  B <- A : hello");
    expect(r.rotateLine("\tA -> B", r.RotateDirection.Left)).toBe("\tB <-- A");
    expect(
      r.rotateLine("[Main] ..> App : use", r.RotateDirection.Swap)
    ).toBe("App <.. [Main] : use");
    expect(
  ...
  it("should rotate an asymetrical arrow", () => {
    expect(r.rotateLine("   B <|-- A", r.RotateDirection.Swap)).toBe("   A --|> B");
  });
  ...
  it("should rotate arrows with multiplicities", () => {
    ...
    expect(
      r.rotateLine('A "0" o-|> "1-2" B', r.RotateDirection.Swap)
    ).toBe('B "1-2" <|-o "0" A');
    expect(
